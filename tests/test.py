import unittest
import subprocess
from maahimai.cli import MaahimAI
...
class TestMaahimAIFramework(unittest.TestCase):
    def test_main_with_agents_advanced(self):
        maahimai = MaahimAI(agent_file='tests/agents-advanced.yaml')
        result = maahimai.run()
...
class TestMaahimAICommandLine(unittest.TestCase):
    def run_command(self, command):
        result = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True, text=True)
        return result.stdout

    def test_maahimai_command(self):
        command = "maahimai --framework autogen --auto create movie script about cat in mars"
        result = self.run_command(command)
        self.assertIn('TERMINATE', result)

    def test_maahimai_init_command(self):
        command = "maahimai --framework autogen --init create movie script about cat in mars"
        result = self.run_command(command)
