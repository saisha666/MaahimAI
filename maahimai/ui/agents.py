...
from maahimaiagents import Agent, Task, MaahimAIAgents, register_display_callback
...
async def step_callback(step_details):
    ...
    try:
        ...
        if step_details.get("tool_name"):
            message_queue.put({
                "content": f" Using tool: {step_details['tool_name']}",
                "author": "System"
            })
    except Exception as e:
...
async def step_callback_wrapper(step_details):
    ...
    try:
        ...
        if step_details.get("tool_name"):
            await cl.Message(
                content=f" {agent_name} is using tool: {step_details['tool_name']}",
                author="System",
            ).send()
...
# -----------------------------------------------------------------------------
# Main MaahimAI Runner
# -----------------------------------------------------------------------------
async def ui_run_maahimai(config, topic, tools_dict):
    logger.info("Starting ui_run_maahimai")
    agents_map = {}
    tasks = []
    tasks_dict = {}
    ...
    try:
        ...
        await cl.Message(content="Starting MaahimAI agents execution...", author="System").send()

        # Decide how to process tasks
        if config.get('process') == 'hierarchical':
            maahim_agents = MaahimAIAgents(
        ...
        cl.user_session.set("agents", maahim_agents)

        loop = asyncio.get_event_loop()
        response = await loop.run_in_executor(None, maahim_agents.start)
        ...
        await cl.Message(content="MaahimAI agents execution completed.", author="System").send()
        await asyncio.sleep(1)
        queue_processor.cancel()
        return result

    except Exception as e:
        error_msg = f"Error in ui_run_maahimai: {str(e)}"
        ...
        settings = await cl.ChatSettings(
            [
                ...
                Select(
                    id="Framework",
                    label="Framework",
                    values=["maahimai", "crewai", "autogen"],
                    initial_index=0,
                ),
            ]
        ).send()
...
async def main(message: cl.Message):
    try:
        ...
        # Run MaahimAI
        result = await ui_run_maahimai(config, message.content, tools_dict)
        ...
        error_msg = f"Error running MaahimAI agents: {str(e)}"
        logger.error(error_msg, exc_info=True)
        await cl.Message(content=error_msg, author="System").send()
