...
class CloudDeployer:
    ...
    def create_dockerfile(self):
        ...
        install the required Python packages (flask, maahimai, gunicorn, and markdown),
        ...
        with open("Dockerfile", "w") as file:
            ...
            file.write("COPY . .\n")
            file.write("RUN pip install flask maahimai gunicorn markdown\n")
    ...
    def create_api_file(self):
        ...
        This method creates an API file named "api.py" in the current directory. The file contains a basic Flask application that uses the MaahimAI library to run a simple agent and returns the output as an HTML page. The application listens on the root path ("/") and uses the Markdown library to format the output.
        """
        with open("api.py", "w") as file:
            file.write("from flask import Flask\n")
            file.write("from maahimai import MaahimAI\n")
            file.write("import markdown\n\n")
            file.write("app = Flask(__name__)\n\n")
            file.write("def basic():\n")
            file.write("    maahimai = MaahimAI(agent_file=\"agents.yaml\")\n")
            file.write("    return maahimai.run()\n\n")
    ...
    def run_commands(self):
        ...
        commands = [
            ...
            "gcloud artifacts repositories create maahimai-repository --repository-format=docker --location=us-central1",
            "docker build --platform linux/amd64 -t gcr.io/$(gcloud config get-value project)/maahimai-app:latest .",
            "docker tag gcr.io/$(gcloud config get-value project)/maahimai-app:latest us-central1-docker.pkg.dev/$(gcloud config get-value project)/maahimai-repository/maahimai-app:latest",
            "docker push us-central1-docker.pkg.dev/$(gcloud config get-value project)/maahimai-repository/maahimai-app:latest",
            "gcloud run deploy maahimai-service --image us-central1-docker.pkg.dev/$(gcloud config get-value project)/maahimai-repository/maahimai-app:latest --platform managed --region us-central1 --allow-unauthenticated --set-env-vars OPENAI_MODEL_NAME=${OPENAI_MODEL_NAME},OPENAI_API_KEY=${OPENAI_API_KEY},OPENAI_API_BASE=${OPENAI_API_BASE}"
        ]
