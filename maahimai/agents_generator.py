...
MAAHIMAI_TOOLS_AVAILABLE = False
...
# Only try to import maahimai_tools if either CrewAI or AutoGen is available
if CREWAI_AVAILABLE or AUTOGEN_AVAILABLE or MAAHIMAI_AVAILABLE:
    try:
        from maahimai_tools import (
...
class AgentsGenerator:
    def __init__(self, agent_file, framework, config_list, log_level=None, agent_callback=None, task_callback=None, agent_yaml=None, tools=None):
        ...
        if framework == "crewai" and not CREWAI_AVAILABLE:
            raise ImportError("CrewAI is not installed. Please install it with 'pip install maahimai[crewai]'")
        elif framework == "autogen" and not AUTOGEN_AVAILABLE:
            raise ImportError("AutoGen is not installed. Please install it with 'pip install maahimai[autogen]'")
        elif framework == "maahimai" and not MAAHIMAI_AVAILABLE:
            raise ImportError("MaahimAI is not installed. Please install it with 'pip install maahimaiagents'")
        ...
        try:
            ...
            return {name: obj() for name, obj in inspect.getmembers(module, 
                lambda x: inspect.isclass(x) and (
                    ...
                    (MAAHIMAI_TOOLS_AVAILABLE and issubclass(x, BaseTool))
    ...
    def generate_crew_and_kickoff(self):
        ...
        # Only try to use maahimai_tools if it's available and needed
        if MAAHIMAI_TOOLS_AVAILABLE and (CREWAI_AVAILABLE or AUTOGEN_AVAILABLE or MAAHIMAI_AVAILABLE):
            tools_dict = {
        ...
        elif framework == "maahimai":
            if not MAAHIMAI_AVAILABLE:
                raise ImportError("MaahimAI is not installed. Please install it with 'pip install maahimaiagents'")
            if AGENTOPS_AVAILABLE:
                agentops.init(os.environ.get("AGENTOPS_API_KEY"), default_tags=["maahimai"])
            return self._run_maahimai(config, topic, tools_dict)
    ...
    def _run_maahimai(self, config, topic, tools_dict):
        """
        Run agents using the MaahimAI framework.
        """
        agents = {}
        tasks = []
        tasks_dict = {}
        ...
        # Create and run the MaahimAI agents
