...
try:
    import maahimai.api.call as call_module
...
try:
    from maahimaiagents import Agent as MaahimAgent, Task as MaahimTask, MaahimAIAgents
    MAAHIMAI_AVAILABLE = True
except ImportError:
    pass
...
class MaahimAI:
    def __init__(self, agent_file="agents.yaml", framework="", auto=False, init=False, agent_yaml=None, tools=None):
        """
        Initialize the MaahimAI object with default parameters.
        """
    ...
    def run(self):
        """
        Run the MaahimAI application.
        """
        self.main()

    def main(self):
        """
        The main function of the MaahimAI object. It parses the command-line arguments,
        initializes the necessary attributes, and then calls the appropriate methods based on the
        provided arguments.
        """
        args = self.parse_args()
        invocation_cmd = "maahimai"
        version_string = f"MaahimAI version {__version__}"
        ...
        if args.command == 'train':
            ...
            if 'init' in sys.argv:
                from maahimai.setup.setup_conda_env import main as setup_conda_main
                setup_conda_main()
            ...
            try:
                ...
                if 'maahimai_env' in result.decode('utf-8'):
                    print("Conda environment 'maahimai_env' found.")
                else:
                    raise subprocess.CalledProcessError(1, 'grep')
            except subprocess.CalledProcessError:
                print("Conda environment 'maahimai_env' not found. Setting it up...")
                from maahimai.setup.setup_conda_env import main as setup_conda_main
                setup_conda_main()
                print("All packages installed.")
            ...
            stream_subprocess(['conda', 'run', '--no-capture-output', '--name', 'maahimai_env', 'python', '-u', train_script_path, 'train'], env=env)
            return
    ...
    def parse_args(self):
        """
        Parse the command-line arguments for the MaahimAI CLI.
        """
        parser = argparse.ArgumentParser(prog="maahimai", description="maahimAI command-line interface")
        parser.add_argument("--framework", choices=["crewai", "autogen", "maahimai"], help="Specify the framework")
        ...
        parser.add_argument("--call", action="store_true", help="Start the MaahimAI Call server")
        ...
        if not args.command and (args.init or args.auto or args.framework):
            if not CREWAI_AVAILABLE and not AUTOGEN_AVAILABLE and not MAAHIMAI_AVAILABLE:
                print("[red]ERROR: No framework is installed. Please install at least one framework:[/red]")
                print("\npip install \"maahimai\\[crewai]\"  # For CrewAI")
                print("pip install \"maahimai\\[autogen]\"  # For AutoGen")
                print("pip install \"maahimai\\[crewai,autogen]\"  # For both frameworks\n")
                print("pip install maahimaiagents # For MaahimAIAgents\n")  
                sys.exit(1)
    ...
    def create_chainlit_chat_interface(self):
        ...
        if CHAINLIT_AVAILABLE:
            ...
            root_path = os.path.join(os.path.expanduser("~"), ".maahimai")
            if "CHAINLIT_APP_ROOT" not in os.environ:
                os.environ["CHAINLIT_APP_ROOT"] = root_path
            chat_ui_path = os.path.join(os.path.dirname(maahimai.__file__), 'ui', 'chat.py')
            chainlit_run([chat_ui_path])
        else:
            print("ERROR: Chat UI is not installed. Please install it with 'pip install \"maahimai[chat]\"' to use the chat UI.")
    ...
    def create_code_interface(self):
        ...
        if CHAINLIT_AVAILABLE:
            ...
            code_ui_path = os.path.join(os.path.dirname(maahimai.__file__), 'ui', 'code.py')
            chainlit_run([code_ui_path])
        else:
            print("ERROR: Code UI is not installed. Please install it with 'pip install \"maahimai[code]\"' to use the code UI.")
    ...
    def create_chainlit_interface(self):
        ...
        if CHAINLIT_AVAILABLE:
            ...
            public_folder = os.path.join(os.path.dirname(maahimai.__file__), 'public')
            if not os.path.exists("public"):
                ...
                logging.info("Public folder already exists.")
            chainlit_ui_path = os.path.join(os.path.dirname(maahimai.__file__), 'ui', 'agents.py')
            chainlit_run([chainlit_ui_path])
        else:
            print("ERROR: Chainlit is not installed. Please install it with 'pip install \"maahimai[ui]\"' to use the UI.")
    ...
    def create_realtime_interface(self):
        ...
        if CHAINLIT_AVAILABLE:
            ...
            if not os.path.exists(os.path.join(root_path, "public")):
                ...
                logging.info("Public folder already exists.")
            realtime_ui_path = os.path.join(os.path.dirname(maahimai.__file__), 'ui', 'realtime.py')
            chainlit_run([realtime_ui_path])
        else:
            print("ERROR: Realtime UI is not installed. Please install it with 'pip install \"maahimai[realtime]\"' to use the realtime UI.")

if __name__ == "__main__":
    maahimai_ai = MaahimAI()
    maahimai_ai.main()
